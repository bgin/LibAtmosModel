...
#include "Config.hpp"
 ...
 ! Tab:10,11 col - Type , function and subroutine code blocks.
    ...
    ! Major version
    integer(I32P), parameter :: MOD_RWSAMPLES_MAJOR = 1
    
    ! Minor version
    integer(I32P), parameter :: MOD_RWSAMPLES_MINOR = 0
    
    ! Micro version
    integer(I32P), parameter :: MOD_RWSAMPLES_MICRO = 0
    
    ! Module full version
    integer(I32P), parameter :: MOD_RWSAMPLES_FULLVER = 1000*MOD_RWSAMPLES_MAJOR+100*MOD_RWSAMPLES_MINOR+ &
...
!DIR$     ASSUME_ALIGNED cexpt:64
          ...
          if(this%m_isbuilt == .true.) then
              if(logging == .true.) then
                  ...
                  call log_UsrMsg("logger:281, In->mod_rwsamples/init: RSWSamples already initialized!!")
                  call log_shutdown()
              else if(verbose == .true.) then
                  call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_rwsamples/init:281, RSWSample_t already initialized!!)"
          ...
          if(nechoes <  1 .OR. &
             ...
             Ts      <= 0._R64P ) then
              if(logging == .true.) then
                   call log_startup(filename,append)
                   call log_UsrMsg("logger:299, In->mod_rwsamples/init: Invalid argument(s) detected!!")
                   call log_shutdown()
              else if(verbose == .true.) then
                    call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_rwsamples/init:299, Invalid argument(s) detected!! )"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          this%m_Ts      = Ts
          this%m_ths     = this%m_Ts/REAL(size,kind=8)
          ! Array members
          ...
          if(aerr /= 0) then
              if(logging == .true.) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:335, In->mod_rwsamples/init: Memory allocation failure!!")
                  call log_shutdown()
              else if(verbose == .true.) then
                  call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================FATAL=========================="
                  write(sdterr,*) " ( mod_rwsamples/init:335, Memory allocation failure!!)"
...
!DIR$         ENDIF
              ERROR STOP "mod_rwsamples/init:335 -> [FATAL-ERROR]: Terminating execution!!"
          end if  
...
!DIR$     SIMD
    ...
    subroutine copy(this,other,logging,filename,append,err)
          ...
          if(this%m_isbuilt == .true.) then
              if(logging == .true.) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:411, In->mod_rwsamples/copy: RSWSamples_t already initialized")
                  call log_shutdown()
              else if (verbose == .true.) then
                  call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_rwsamples/copy:411, RSWSample_t already initialized!!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
    ...
    subroutine destroy(this,logging,filename,append,err)
          ...
          if(this%m_isbuilt == .false.) then
              if(logging == .true.) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:480, In->mod_rwsamples/destroy: RSWSamples already destroyed")
                  call log_shutdown()
              else if(verbose == .true.) then
                  call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_rwsamples/destroy:480, RSWSample_t already initialized!!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(derr /= 0) then
              if(logging == .true.) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:510,In->mod_rwsamples/destroy: Memory deallocation failure!!")
                  call log_shutdown()
              else if(verbose == .true.) then
                  call DATE_AND_TIME(date=sdate,time=stime)
                  write(stderr,*) "===========================FATAL=========================="
                  write(stderr,*) " ( mod_rwsamples/destroy:510, Memory deallocation failure)"
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_rwsamples/destroy:510 -> [FATAL-ERROR]: Terminating execution!!"
          end if
...
!DIR$     IF (USE_IEEE_EXCEPTION_HANDLING .EQ. 1)
          if(present(fpflags) == .true.) then
              ...
              WRITE(stderr,*) "mod_rwsamples/compute_Vsamp:792 -- optional parameter: fpflags - not present!!"
          end if
!DIR$     ENDIF   
          do j = 1, this%m_size
!DIR$     SIMD
              ...
              call ieee_get_flag(ieee_all,fpflags)
                 if(ANY(fpflags) == .true.) then
                     if(present(verbose) .AND. verbose == .true.) then
                          WRITE(stderr,*) "=============================================================================="
                          WRITE(stderr,*) "  mod_rwsamples/compute_Vsamp:809 -- Floating-point exception(s) occurred!!"
                          WRITE(stderr,*) "==============================================================================="
                     end if
                     err = -3
                 end if
...
!DIR$     IF (USE_IEEE_EXCEPTION_HANDLING .EQ. 1)
          if(present(fpflags) == .true.) then
              ...
              WRITE(stderr,*) "mod_rwsamples/compute_Vsampavg:874 -- Optional argument: fpflags -- not present!!"
          end if
!DIR$     ENDIF   
          do j = 1, this%m_size
!DIR$     SIMD
          ...
          if(present(fpflags) == .true.) then
              call ieee_get_flag(ieee_all,fpflags)
                 if(ANY(fpflags) == .true.) then
                     if(present(verbose) .AND. verbose == .true.) then
                          WRITE(stderr,*) "=============================================================================="
                          WRITE(stderr,*) "  mod_rwsamples/compute_Vsampavg:8942-- Floating-point exception(s) occurred!!"
                          WRITE(stderr,*) "==============================================================================="
                     end if
                     err = -3
                 end if
...
!DIR$     ENDIF           
    ...
    subroutine assign_samples(this,other)
          ...
          if(LOC(this) == LOC(other)) then
               call DATE_AND_TIME(date=sdate,time=stime)
               write(stderr,*) "===========================NON-FATAL=========================="
               write(sdterr,*) " ( mod_rwsamples/assignment(=):959, Attempted self_assignment!)"
               write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
               write(stderr,*) "===========================NON-FATAL=========================="
    ...
    function samples_neq_samples(s1,s2) result(neq)
          ...
          implicit none
          type(RSWSamples_t), intent(in) :: s1,s2
          ! Locals
          type(bp2) :: neq
          ! Start of executable statements
          bp2%x = s1%m_Vsamp    /= s2%m_Vsamp
          bp2%y = s1%m_Vsampavg /= s2%m_Vsampavg
    end function
