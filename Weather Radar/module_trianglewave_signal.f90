...
!DIR$     ASSUME_ALIGNED noise:32,r:32
          complex(R64P), dimension(:), intent(in)    :: h,v
          logical(I32P),               intent(in)    :: logging
          character(len=*),            intent(in)    :: filename
          logical(I32P),               intent(in)    :: append,dbg,profiling
          ...
          if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:637, In->mod_trianglew_signal/phase_noise_signal: TriangleWSignal_t already initialized!!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/phase_noise_signal:637, TriangleWSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(invalid) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:660, In->mod_trianglew_signal/phase_noise_signal: Invalid time-step increment!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/phase_noise_signal:660, Invalid time-step increment!!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(dur.LE.LAM_ZR8) then
              ...
              if(logging) then
                   call log_startup(filename,append)
                   call log_UsrMsg("logger:684, In->mod_trianglew_signal/phase_noise_signal: Invalid arguments: dur!")
                   call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/phase_noise_signal:684, Invalid arguments: dur  !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:727, In->mod_trianglew_signal/phase_noise_signal: Memory allocation failure!!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_trianglew_signal/phase_noise_signal:727, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_trianglew_signal/phase_noise_signal:727 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
                  write(stderr,*) "mod_trianglew_signal/phase_noise_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
           ! Create Jones vector field.
          do i = 1, this%m_nsamp
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail .EQ. .false.) then
                         ...
                         write(stderr,*) "mod_trianglew_signal/phase_noise_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_trianglew_signal/phase_noise_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
    end subroutine
...
!DIR$     ASSUME_ALIGNED h:32,v:32
          ...
          if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:867, In->mod_trianglew_signal/additive_noise_signal: TriangleWSignal_t already initialized!!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/additive_noise_signal:867, TriangleWSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(invalid) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:890, In->mod_trianglew_signal/additive_noise_signal: Invalid time-step increment!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/phase_noise_signal:890, Invalid time-step increment!!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(dur.LE.LAM_ZR8) then
              ...
              if(logging) then
                   call log_startup(filename,append)
                   call log_UsrMsg("logger:914, In->mod_trianglew_signal/aditive_noise_signal: Invalid arguments: dur!")
                   call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_trianglew_signal/phase_noise_signal:914, Invalid arguments: dur  !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:956, In->mod_trianglew_signal/additive_noise_signal: Memory allocation failure!!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_trianglew_signal/additive_noise_signal:956, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_trianglew_signal/phase_noise_signal:956 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
                  write(stderr,*) "mod_trianglew_signal/additive_noise_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
            ! Create Jones vector field.
          do i = 1, this%m_nsamp
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail .EQ. .false.) then
                         ...
                         write(stderr,*) "mod_trianglew_signal/additive_noise_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_trianglew_signal/additive_noise_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
    end subroutine
