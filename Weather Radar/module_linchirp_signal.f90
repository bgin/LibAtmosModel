...
module mod_linchirp_signal
 ...
 !                        Time: 09:17 GMT+2
 !           Modified:
 !                      By Bernard Gingold on: 10-03-2018, 14:49 GMT+2
...
!DIR$   ATTRIBUTES ALIGN : 32 :: m_S
        ...
        ! Main constructor
        procedure, pass(this), public :: linchirp_signal
        
       
        ! Creates signal with noise modulated phase
        procedure, pass(this), public :: phase_noise_linchirp_signal
        
        
        ! Creates signal with noise modulated phase
        ! coupled with background additive noise.
        procedure, pass(this), public :: additive_noise_linchirp_signal
    ...
    interface LinChirpSignal_t
         procedure :: constructor
    end interface LinChirpSignal_t
    ...
    !======================================================60
    type(LinChirpSignal_t) function constructor(name,sid,nsamp,logging, &
          ...
          if(aerr /= 0) then
              ...
              call handle_fatal_memory_error(logging, verbose, append,filename,  &
                                             "logger:297 --> mod_linchirp_signal/constructor: Memory Allocation Failure!!" , &
                                             "mod_linchirp_signal/constructor:297 -- Memory Allocation Failure!" ,  &
                                             emsg,__LINE__ )
          end if
    ...
    !======================================================60
    !  @subroutine: linchirp_signal                        
    !              Physical representation of complex linear
    !              chirp signal.            
    !======================================================60 
    subroutine linchirp_signal(this,ierr,dur,sfreq,   &
                             efreq,chrate,siphi,sinit,stsinc, &
                             r,h,v,verbose,logging,filename,append,   &
                             profiling,qpctimer          )
          use mod_print_error,  only : handle_fatal_memory_error
          use mod_constants,    only : LAM_PINF, LAM_c, LAM_ZINIT, LAM_NS, LAM_HR64P, LAM_2PI
...
!DIR$     ATTRIBUTES ALIGN : 64 :: tmp
          ...
          if(aerr /= 0) then
              ...
              call handle_fatal_memory_error(logging,verbose,append,filename,   &
                                             "logger:375 --> mod_linchirp_singnal/linchirp_signal: Memory Allocation Failure!!"  , &
                                             "mod_linchirp_signal/linchirp_signal:375 -- Memory Allocation Failure!!",  &
                                             emsg,__LINE__ )
          end if
          ct = LAM_PINF
          ...
          if(profiling) then
              ...
              if(ifail == 0) then
                  write(stderr,*) "mod_linchirp_signal/linchirp_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail == .false.) then
                         ...
                         write(stderr,*) "mod_linchirp_signal/linchirp_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_linchirp_signal/linchirp_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
   end subroutine linchirp_signal      
    ...
    !======================================================60
    !  @subroutine: phase_noise_linchirp_signal                        
    !              Physical representation of Linear Chirp
    !              signal.
    !              This signal is corrupted by phase additive
    !              noise.       
    !======================================================60                         
    subroutine phase_noise_linchirp_ignal(this,ierr,dur,sfreq,   &
                                  efreq,chrate,siphi,sinit,stsinc, &
                                  phnoise,r,h,v,verbose,logging,filename,append,   &
                                  profiling,qpctimer  )
          use mod_print_error,  only : handle_fatal_memory_error
          use mod_constants,    only : LAM_PINF, LAM_c, LAM_ZINIT, LAM_NS, LAM_HR64P, LAM_2PI
          class(LinChirpSignal_t),     intent(inout) :: this
...
!DIR$     ATTRIBUTES ALIGN : 64 :: tmp
          ...
          if(aerr /= 0) then
              ...
              call handle_fatal_memory_error(logging,verbose,append,filename,   &
                                             "logger:510 --> mod_linchirp_singnal/phase_noise_linchirp_signal: Memory Allocation Failure!!" , &
                                             "mod_linchirp_signal/phase_noise_linchirp_signal:510 -- Memory Allocation Failure!" , &
                                             emsg,__LINE__ )
          end if
          ct = LAM_PINF
          tmp = CMPLX(LAM_PINF,LAM_PINF,R64P)
          ...
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail == 0) then
                  write(stderr,*) "mod_linchirp_signal/phase_noise_linchirp_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ! Construct electric field
          do i = 1_I32P, this%m_nsamp
          ...
          if(profiling) then
                if(ifail == 0) then
                    ...
                    if(bfail == .false.) then
                         ...
                         write(stderr,*) "mod_linchirp_signal/phase_noise_linchirp_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_linchirp_signal/phase_noise_linchirp_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
         
    end subroutine  phase_noise_linchirp_signal
                                  
    !======================================================60
    !  @subroutine: additive_noise_linchirp_signal                        
    !              Physical representation of Linear Chirp
    !              signal.
    !              This signal is corrupted by phase additive
    !              noise and by background additive noise.      
    !======================================================60 
    subroutine additive_noise_linchirp_signal(this,ierr,dur,sfreq,   &
                                     efreq,chrate,siphi,sinit,stsinc, &
                                     phnoise,bnoise,r,h,v,verbose,logging,filename,append,   &
                                     profiling,qpctimer   )
          use mod_print_error,  only : handle_fatal_memory_error
          use mod_constants,    only : LAM_PINF, LAM_c, LAM_ZINIT, LAM_NS, LAM_HR64P, LAM_2PI
          class(LinChirpSignal_t),     intent(inout) :: this
...
!DIR$     ATTRIBUTES ALIGN : 64 :: tmp
          ...
          if(aerr /= 0) then
              ...
              call handle_fatal_memory_error(logging,verbose,append,filename,   &
                                             "logger:650 --> mod_linchirp_singnal/additive_noise_linchirp_signal: Memory allocation failure!!" , &
                                             "mod_linchirp_signal/phase_noise_linchirp_signal:650 -- Memory Allocation Failure!",  &
                                             emsg,__LINE__ )
          end if
          ct = LAM_PINF
          tmp = CMPLX(LAM_PINF,LAM_PINF,R64P)
          ...
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail == 0) then
                  write(stderr,*) "mod_linchirp_signal/additive_noise_linchirp_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ! Construct electric field
          do i = 1_I32P, this%m_nsamp
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail == .false.) then
                         ...
                         write(stderr,*) "mod_linchirp_signal/additive_noise_linchirp_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_linchirp_signal/additive_noise_linchirp_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
       
    end subroutine  additive_noise_linchirp_signal
...
!DIR$     ATTRIBUTES ALIGN : 64 :: w
          ...
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail == 0) then
                  write(stderr,*) "mod_expchirp_signal/analytic_signal:858, qpctimer_start failed to query performance counter!"
              end if
          end if
...
!DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
          ...
          if(profiling) then
               if(ifail.EQ.0) then
                   ...
                   if(bfail == .false.) then
                        ...
                        write(stderr,*) "mod_linchirp_signal/analytic_signal:901, qpctimer_delta: failed to compute delta measurement!!"
                   end if  
               else
                       write(stderr,*) "module_linchirp_signal/analytic_signal:901 Unable to read performance counter -- fatal!!"
               end if
          end if
    ...
    subroutine copy_assign(lhs,rhs)
          class(LinChirpSignal_t),  intent(inout) :: lhs
          class(LinChirpSignal_t),  intent(in)    :: rhs
          ! Start of executable statements
