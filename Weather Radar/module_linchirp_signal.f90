...
!DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
    ...
    !              Physical representation of Linear Chirp
    !              signal.
    !              This signal is corrupted by phase additive
    !              noise.       
    !======================================================60                         
    subroutine phase_noise_signal(this,name,sid,nsamp,dur,sfreq,   &
                                  efreq,chrate,siphi,sinit,stsinc, &
                                  phnoise,r,h,v,logging,filename,append,   &
                                  dbg,profiling,qpctimer  )
          implicit none
          class(LinChirpSignal_t),     intent(inout) :: this
          character(len=*),            intent(in)    :: name
...
!DIR$     ASSUME_ALIGNED phnoise:32,r:32
          ...
          integer(BOOL)      :: ifail
          logical(I32P)      :: bfail
          real(R64P)         :: insamp,t,t2,delta,tc,ts,tn
          real(R64P),    allocatable, dimension(:) :: ct
!DIR$     ATTRIBUTES ALIGN : 32 :: ct
          complex(R64P), allocatable, dimension(:) :: tmp
!DIR$     ATTRIBUTES ALIGN : 32 :: tmp
          ! Start of executable statements
           if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:613, In->mod_linchirp_signal/phase_noise_signal: LinChirpSignal_t already initialized!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                   write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_linchirp_signal/phase_noise_signal:613, LinChirpSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(efreq.LE.sfreq) then
               if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:632, In->mod_linchirp_signal/phase_noise_signal: f1 <= f0 !!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_linchirp_signal/phase_noise_signal:632, f1 <= f0 !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:679, In->mod_linchirp_singnal/phase_noise_signal: Memory allocation failure!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_linchirp_signal/phase_noise_signal:679, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/phase_noise_signal:679 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          !DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
          do i = 1, this%m_nsamp
              this%m_jvec(i)    = JonesVector_t(h(i),v(i)) 
          ...
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
                  write(stderr,*) "mod_linchirp_signal/phase_noise_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ! Construct electric field
          do i = 1, this%m_nsamp
              ...
              t2 = t-r(i)/LAM_c
              tn = t2+phnoise(i)
              this%m_phi(i) = (LAM_HR64P*this%m_chrate*tn*tn+this%m_sfreq)*LAM_2PI+this%m_siphi
          ...
          do i = 1, this%m_nsamp
              ...
              this%m_phi(i) = (LAM_HR64P*this%m_chrate*tn*tn+this%m_sfreq) * &
                               LAM_2PI+this%m_siphi
              tmp(i) = DCMPLX(DCOS(this%m_phi(i)),DSIN(this%m_phi(i)))
          end do
          ! Compute natural envelope
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail .EQ. .false.) then
                         ...
                         write(stderr,*) "mod_linchirp_signal/phase_noise_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_linchirp_signal/phase_noise_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
          if(dbg) then
              print*, "Jones vec field ",  this%m_jvec
    ...
    !======================================================60 
    subroutine additive_noise_signal(this,name,sid,nsamp,dur,sfreq,   &
                                     efreq,chrate,siphi,sinit,stsinc, &
                                     phnoise,bnoise,r,h,v,logging,filename,append,   &
                                     dbg,profiling,qpctimer   )
          implicit none
          class(LinChirpSignal_t),     intent(inout) :: this
          character(len=*),            intent(in)    :: name
...
!DIR$     ATTRIBUTES ALIGN : 32 :: tmp
          ! Start of executable statements
           if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:831, In->mod_linchirp_signal/additive_noise_signal: LinChirpSignal_t already initialized!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                   write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_linchirp_signal/additive_noise_signal:831, LinChirpSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(efreq.LE.sfreq) then
               if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:847, In->mod_linchirp_signal/additive_noise_signal: f1 <= f0 !!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_linchirp_signal/phase_noise_signal:847, f1 <= f0 !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:893, In->mod_linchirp_singnal/additive_noise_signal: Memory allocation failure!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_linchirp_signal/phase_noise_signal:893, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/phase_noise_signal:893 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          !DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
          do i = 1, this%m_nsamp
              this%m_jvec(i)    = JonesVector_t(h(i),v(i)) 
          ...
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
                  write(stderr,*) "mod_linchirp_signal/additive_noise_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ! Construct electric field
          do i = 1, this%m_nsamp
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail .EQ. .false.) then
                         ...
                         write(stderr,*) "mod_linchirp_signal/additive_noise_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_linchirp_signal/additive_noise_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
          if(dbg) then
              print*, "Jones vec field ",  this%m_jvec
    ...
    subroutine destroy_signal(this,logging,filename,append)
          ...
          if(ALLOCATED(this%m_jvec)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:676, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_jvec]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:676, Deallocation of [m_jvec]: failed)"
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:676, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_E)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:701, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_E]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:701, Deallocation of [m_E]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:701, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_nenvp)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:726, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_nenvp]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:726, Deallocation of [m_nenvp]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:726, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
          if(ALLOCATED(this%m_phi)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:751, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_phi]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:751, Deallocation of [m_nenvp]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:751, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
          if(ALLOCATED(this%m_canform)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      ...
                      call log_UsrMsg("logger:776, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_canform]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:776, Deallocation of [m_canform]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:776, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
          if(ALLOCATED(this%m_basform)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:801, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_basform]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:801, Deallocation of [m_basform]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:801, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
          if(ALLOCATED(this%m_cenvp)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:826, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_cenvp]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:826, Deallocation of [m_cenvp]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:826, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
          if(ALLOCATED(this%m_amp)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:851, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_amp]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:851, Deallocation of [m_amp]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:851, [FATAL-ERROR]: Terminating execution!"
              end if
          end if  
          if(ALLOCATED(this%m_S)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:876, In->mod_linchirp_signal/destroy_signal: Deallocation of [m_S]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================="
                      write(stderr,*) "  ( mod_linchirp_signal/destroy_signal:876, Deallocation of [m_S]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_linchirp_signal/destroy_signal:876, [FATAL-ERROR]: Terminating execution!"
              end if
          end if 
