...
!DIR$         ENDIF
    ...
    !              noise.       
    !======================================================60
    subroutine phase_noise_signal(this,name,sid,nsamp,maxk,dur,A,   &
...
!DIR$     ASSUME_ALIGNED h:32,v:32
          ...
          if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:621, In->mod_sawtooth_signal/phase_noise_signal: SawtoothSignal already initialized!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_sawtooth_signal/phase_noise_signal:621, SawtoothSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(dur.LE.LAM_ZR8 .OR. &
             A.LE.LAM_ZR8 ) then
              if(logging) then
                   call log_startup(filename,append)
                   call log_UsrMsg("logger:643, In->mod_sawtooth_signal/phase_noise_signal: Invalid arguments i.e. A or dur!")
                   call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_sawtooth_signal/phase_noise_signal:643, Invalid arguments i.e. A or dur  !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:691, In->mod_sawtooth_singnal/phase_noise_signal: Memory allocation failure!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_sawtoth_signal/phase_noise_signal:691, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/phase_noise_signal:691 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
    ...
    !======================================================60 
    subroutine additive_noise_signal(this,name,sid,nsamp,maxk,dur,A,        &
...
!DIR$     ASSUME_ALIGNED h:32,v:32
          ...
          if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:831, In->mod_sawtooth_signal/additive_noise_signal: SawtoothSignal already initialized!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_sawtooth_signal/additive_noise_signal:831, SawtoothSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(dur.LE.LAM_ZR8 .OR. &
             A.LE.LAM_ZR8 ) then
              if(logging) then
                   call log_startup(filename,append)
                   call log_UsrMsg("logger:853, In->mod_sawtooth_signal/additive_noise_signal: Invalid arguments i.e. A or dur!")
                   call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_sawtooth_signal/additive_noise_signal:853, Invalid arguments i.e. A or dur  !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:901, In->mod_sawtooth_singnal/additive_noise_signal: Memory allocation failure!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
                  write(ERROR_UNIT,*) "   (mod_sawtoth_signal/additive_noise_signal:901, Memory allocation failure!)"
                  write(ERROR_UNIT,*) "   (System message:)", emsg
                  write(ERROR_UNIT,*) "   ( Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/additive_noise_signal:901 -> [FATAL-ERROR]: Terminating execution!!"
          end if
          if(profiling) then
              call qpctimer_start(qpctimer,ifail)
              if(ifail.EQ.0) then
                  write(stderr,*) "mod_sawtooth_signal/additive_noise_signal: qpctimer_start failed to query performance frequency counter!"
              end if
          end if
          ! Set up jones vector field
          do j = 1, this%m_nsamp
          ...
          if(profiling) then
                if(ifail.EQ.0) then
                    ...
                    if(bfail .EQ. .false.) then
                         ...
                         write(stderr,*) "mod_sawtooth_signal/additive_noise_signal: qpctimer_delta: failed to compute delta measurement!!"
                    end if  
                else
                    write(stderr,*) "mod_sawtooth_signal/additive_noise_signal: Unable to read performance counter -- fatal!!"
                end if
          end if
    end subroutine
    !======================================================60                         
    ...
    subroutine copy_signal(this,other,logging,filename,append)
          ...
          if(LOC(this).EQ.LOC(other) .OR. &
             this%m_isbuilt .EQ. .true. ) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:1020, In->mod_sawtooth_signal/copy_signal: Attempted self-assignment, or argument in invalid state!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_sawtooth_signal/copy_signal:1020, Attempted self_assignment, or argument in invalid state!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
    ...
    subroutine destroy_signal(this,logging,filename,append)
          ...
          if(this%m_isbuilt .EQ. .false.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:1074, In->mod_sawtooth_signal/destroy_signal: SawtoothSignal_t already destroyed!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL============================================="
                  write(sdterr,*) " ( mod_sawtooth_signal/destroy_signal:1074, SawtoothSignal_t already destroyed!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL============================================="
          ...
          do j = 1, this%m_nsamp
                 call this%m_jvec(j)%destroy_jvec
          end do
          ...
          if(ALLOCATED(this%m_jvec)) then
              deallocate(this%m_jvec, &
                ...
                if(derr.NE.0) then
                    if(logging) then
                       call log_startup(filename,append)
                       call log_UsrMsg("logger:1110, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_jvec]: failed!")
                       call log_shutdown()
                  else
                       call DATE_AND_TIME(date=dstr,time=tstr)
                       write(stderr,*) "=========================FATAL=================================================="
                       write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:660, Deallocation of [m_jvec]: failed)"
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1110, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_scomp)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                       call log_startup(filename,append)
                       call log_UsrMsg("logger:1135, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_scomp]: failed!")
                       call log_shutdown()
                  else
                       call DATE_AND_TIME(date=dstr,time=tstr)
                       write(stderr,*) "=========================FATAL=================================================="
                       write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1135, Deallocation of [m_scomp]: failed)"
                       write(stderr,*) "   System: ", emsg
                       write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1135, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_ccomp)) then
              deallocate(this%m_ccomp, &
                ...
                if(derr.NE.0) then
                    if(logging) then
                        call log_startup(filename,append)
                        call log_UsrMsg("logger:1156, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_ccomp]: failed!")
                        call log_shutdown()
                    else
                        call DATE_AND_TIME(date=dstr,time=tstr)
                        write(stderr,*) "=========================FATAL===================================================="
                        write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1156, Deallocation of [m_ccomp]: failed)"
                        write(stderr,*) "   System: ", emsg
                        write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1156, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_wct)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:1185, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_wct]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL================================================"
                      write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1185, Deallocation of [m_wct]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1185, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_nenvp)) then
              deallocate(this%m_nenvp,  &
               ...
               if(derr.NE.0) then
                   if(logging) then
                       call log_startup(filename,append)
                       call log_UsrMsg("logger:1210, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_nenvp]: failed!")
                       call log_shutdown()
                   else
                       call DATE_AND_TIME(date=dstr,time=tstr)
                       write(stderr,*) "=========================FATAL==================================================="
                       write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1210, Deallocation of [m_nenvp]: failed)"
                       write(stderr,*) "   System: ", emsg
                       write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1210, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_sawtooth)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:1235, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_sawtooth]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL======================================================"
                      write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1235, Deallocation of [m_sawtooth]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1235, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_cform)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:1260, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_cform]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL==================================================="
                      write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1260, Deallocation of [m_cform]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1260, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_E)) then
               ...
               if(derr.NE.0) then
                   if(logging) then
                       call log_startup(filename,append)
                       call log_UsrMsg("logger:1285, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_E]: failed!")
                       call log_shutdown()
                   else
                       call DATE_AND_TIME(date=dstr,time=tstr)
                       write(stderr,*) "=========================FATAL==============================================="
                       write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1285, Deallocation of [m_E]: failed)"
                       write(stderr,*) "   System: ", emsg
                       write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1285, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_amp)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:1310, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_amp]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL==================================================="
                      write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1310, Deallocation of [m_amp]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1310, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
          if(ALLOCATED(this%m_S)) then
              ...
              if(derr.NE.0) then
                  if(logging) then
                      call log_startup(filename,append)
                      call log_UsrMsg("logger:1335, In->mod_sawtooth_signal/destroy_signal: Deallocation of [m_S]: failed!")
                      call log_shutdown()
                  else
                      call DATE_AND_TIME(date=dstr,time=tstr)
                      write(stderr,*) "=========================FATAL==================================================="
                      write(stderr,*) "  ( mod_sawtooth_signal/destroy_signal:1335, Deallocation of [m_S]: failed)"
                      write(stderr,*) "   System: ", emsg
                      write(stderr,*) "  ( Fatal Error at:) ",  &
                                     dstr(1:4),"-",dstr(5:6),"-",dstr(7:8), " ", &
                                     tstr(1:2),":",tstr(3:4),":",tstr(5:6)
...
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_sawtooth_signal/destroy_signal:1335, [FATAL-ERROR]: Terminating execution!"
              end if
          end if
