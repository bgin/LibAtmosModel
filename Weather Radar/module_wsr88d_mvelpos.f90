...
module mod_wsr88d_mvelpos
 ...
 ! Tab:10,11 col - Type , function and subroutine code blocks.
    ...
    subroutine init(this,na,nr,ierr,logging,verbose,dbg,append,fname)
          ...
          implicit none 
          class(MaxVelPos_t), intent(inout) :: this
          ...
          if(this%m_isbuilt == .true. .OR.   &
             na <= 0_I32P .OR.  nr <= 0_I32P ) then
              if(logging == .true. ) then
                  ...
                  call log_UsrMsg("logger:182 --> mod_wsr88d_mvelpos/init: MaxVelPos_t already initialized, or invalid array size argument!!")
                  call log_shutdown()
              else if (verbose == .true.) then
                  call print_non_fatal_error("========================= NON_FATAL =========================", &
                                             "mod_wsr88d_mvelpos/init:182 -- MaxVelPos_t already initialized, or invalid array size argument!!", &
                                             sdate,stime,182)
              end if
              ierr = -1_I32P
              return
          ...
          if(aerr /= 0)  then
              if(logging == .true.) then
                  call log_startup(fname,append)
                  call log_UsrMsg("logger:206 --> mod_wsr88d_mvelpos/init: Memory Allocation Failure")
                  call log_shutdown()
              else if (verbose == .true.) then
                  call print_fatal_error("========================= FATAL =========================", &
                                         " mod_wsr88d_mvelpos/init: Memory Allocation Failure ",      &
                                         emsg, sdate, stime, 206)
              end if
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP "mod_wsr88d_mvelpos/init:206 [FATAL-ERROR] -- Memory Allocation Failure !!"
          end if
          ! Initialize arrays
...
!DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
    ...
    subroutine copy(this,other,logging,fname,append,verbose,ierr)
          ...
          implicit none
          class(MaxVelPos_t), intent(inout) :: this
          class(MaxVelPos_t), intent(in)    :: other
          ...
          ! Locals
          character(len=40) :: sdate, stime
          ! Start of exeutable statements
          if(ierr <= 0_I32P) ierr = 0_I32P
          if(this%m_isbuilt == .true. .OR. &
             other%m_isbuilt == .false.  ) then
              if(logging == .true.) then
                  call log_startup(fname,append)
                  call log_UsrMsg("logger:266 --> mod_wsr88d_mvelpos/init: Invalid arguments state!!")
                  call log_shutdown()
              else if (verbose == .true.) then
                  call print_non_fatal_error("========================= NON-FATAL =========================",  &
                                             "mod_wsr88d_mvelpos/init266:  [Invalid arguments state!!]" ,     &
                                             sdate, stime, 266)
              end if
              ierr = -1_I32P
              return
    ...
    subroutine destroy(this,logging,fname,append,verbose,ierr)
          ...
          if(this%m_isbuilt == .false.) then
              if(logging == .true.) then
                  call log_startup(fname,append)
                  call log_UsrMsg("logger:318 --> mod_wsr88d_mvelpos/init: MaxVelPos_t already destroyed!!")
                  call log_shutdown()
              else if (verbose == .true.) then
                  call print_non_fatal_error("========================= NON-FATAL =========================", &
                                             "mod_wsr88d_mvelpos/init:318 --> MaxVelPos_t already destroyed!!" , &
                                             sdate, stime, 318)
              end if
              ierr = -1_I32P
              return
          ...
          if(derr /= 0) then
              if(logging == .true.) then
                  call log_startup(fname, append)
                  call log_UsrMsg("logger:337 --> mod_wsr88d_mvelpos/init: Memory Deallocation Failure!!")
                  call log_shutdown()
              else if(verbose == .true.) then
                  call print_fatal_error( "========================= FATAL =========================" , &
                                          "mod_wsr88d_mvelpos/init337: Memory Deallocation Failure!!" , &
                                           sdate, stime, 337)
              end if
!DIR$         IF (SHOW_CALLSTACK .EQ. 1)
              call TRACEBACKQQ(STRING="FATAL-ERROR", USER_EXIT_CODE= -1)
!DIR$         ENDIF
              ERROR STOP  "mod_wsr88d_mvelpos/init337: [FATAL-ERROR] --> Memory Deallocation Failure!!"
          end if
