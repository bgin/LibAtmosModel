...
module mod_expchirp_signal
 ...
 ! Tab:10,11 col - Type , function and subroutine code blocks.
    ...
    type :: ExpChirpSignal_t
         ...
         character(len=32)   :: m_ctors     ! name of constructor which created this
         
         type(JonesVector_t) :: m_jvec      ! Jones Vector (polarization)
...
!DIR$     ASSUME_ALIGNED r:32          
          ...
          if(this%m_isbuilt .EQ. .true.) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:381, In->mod_expchirp_signal/create_signa: ExpChirpSignal_t already initialized!!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_expchirp_signal/create_signal:261, ExpChirpSignal_t already initialized!)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(efreq.LE.sfreq) then
               if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:400, In->mod_expchirp_signal/create_signal: f1 <= f0 !!"
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(stderr,*) "===========================NON-FATAL=========================="
                  write(sdterr,*) " ( mod_expchirp_signal/create_signal:261, f1 <= f0 !)"
                  write(stderr,*) " ( Non-Fatal Error at:) ",  &
                                     sdate(1:4),"-",sdate(5:6),"-",sdate(7:8), " ", &
                                     stime(1:2),":",stime(3:4),":",stime(5:6)
                  write(stderr,*) "===========================NON-FATAL=========================="
          ...
          if(aerr.NE.0) then
              if(logging) then
                  call log_startup(filename,append)
                  call log_UsrMsg("logger:445, In->mod_expchirp_signal/default_signal: Memory allocation failure!")
                  call log_shutdown()
              else
                  call DATE_AND_TIME(date=dstr,time=tstr)
                  write(ERROR_UNIT,*) "===========================FATAL========================="
...
!DIR$     ASSUME_ALIGNED asig:32
          ...
          if(profiling) then
             call qpctimer_start(qpctimer,ifail)
             if(ifail.EQ.0) then
                 write(stderr,*) "mod_expchirp/analytic_signal:996, qpctimer_start failed to query performance frequency counter!!"
             end if
          end if
!DIR$     SIMD VECTORLENGTHFOR(REAL(KIND=8))
          do i = 0, tmplen
