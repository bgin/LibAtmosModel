...
!-----------------------------------------------------------------------------------------------------------------------------------
 use module_kinds, only : I8P,I16P,I32P,I64P,R32P,R64P
!-----------------------------------------------------------------------------------------------------------------------------------
implicit none
private
public:: b64_encode
...
contains
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  real(R64P),                 intent(IN)::    a1(1:)    !< Firs data stream.
  real(R32P),                 intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  integer(I32P)::                             np        !< Size of temporary packed data.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  real(R64P),                 intent(IN)::    a1(1:)    !< First data stream.
  integer(I64P),              intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  integer(I32P)::                             np        !< Size of temporary packed data.
  ...
  implicit none
  real(R64P),                 intent(IN)::    a1(1:)    !< First data stream.
  integer(I32P),              intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  integer(I32P)::                             np        !< Size of temporary packed data.
  ...
  implicit none
  real(R64P),                 intent(IN)::    a1(1:)    !< First data stream.
  integer(I16P),              intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  integer(I32P)::                             np        !< Size of temporary packed data.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  real(R32P),                 intent(IN)::    a1(1:)    !< Firs data stream.
  real(R64P),                 intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  integer(I32P)::                             np        !< Size of temporary packed data.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  real(R32P),                 intent(IN)::    a1(1:)    !< First data stream.
  integer(I64P),              intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  integer(I64P),              intent(IN)::    a1(1:)    !< First data stream.
  real(R64P),                 intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  integer(I32P),              intent(IN)::    a1(1:)    !< First data stream.
  real(R64P),                 intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  integer(I16P),              intent(IN)::    a1(1:)    !< First data stream.
  real(R64P),                 intent(IN)::    a2(1:)    !< Second data stream.
  integer(I8P), allocatable, intent(INOUT):: packed(:) !< Packed data into I1P array.
  integer(I8P), allocatable::                p1(:)     !< Temporary packed data of first stream.
  integer(I8P), allocatable::                p2(:)     !< Temporary packed data of second stream.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  integer(I8P), intent(IN)::  bits(1:)  !< Bits to be encoded.
  integer(I32P), intent(IN)::  padd      !< Number of padding characters ('=').
  character(1), intent(OUT):: code(1:)  !< Characters code.
  integer(I8P)::              sixb(1:4) !< 6 bits slices (stored into 8 bits integer) of 24 bits input.
  integer(I64P)::              c,e       !< Counters.
  !---------------------------------------------------------------------------------------------------------------------------------

  !---------------------------------------------------------------------------------------------------------------------------------
  c = 1_I64P
  do e=1_I64P,size(bits,dim=1),3_I64P ! loop over array elements: 3 bytes (24 bits) scanning
    sixb = 0_I8P
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  integer(I32P),              intent(IN)::  nB                                 !< Number of bytes of single element of n.
  real(R64P),                 intent(IN)::  n(1:)                              !< Array of numbers to be encoded.
  character(1), allocatable, intent(OUT):: code(:)                            !< Encoded array.
  integer(I8P)::                           nI1P(1:((size(n,dim=1)*nB+2)/3)*3) !< One byte integer array containing n.
  integer(I32P)::                           padd                               !< Number of padding characters ('=').
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  padd = mod((size(n,dim=1)*nB),3_I4P) ; if (padd>0_I4P) padd = 3_I32P - padd           ! computing the number of padding characters
  ...
  implicit none
  integer(I32P),              intent(IN)::  nB                                 !< Number of bytes of single element of n.
  real(R32P),                 intent(IN)::  n(1:)                              !< Array of numbers to be encoded.
  character(1), allocatable, intent(OUT):: code(:)                            !< Encoded array.
  integer(I8P)::                           nI1P(1:((size(n,dim=1)*nB+2)/3)*3) !< One byte integer array containing n.
  integer(I32P)::                           padd                               !< Number of padding characters ('=').
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  if (allocated(code)) deallocate(code) ; allocate(code(1:((size(n,dim=1)*nB+2)/3)*4)) ! allocating code chars
  nI1P = transfer(n,nI1P)                                                              ! casting n to integer array of 1 byte elem
  padd = mod((size(n,dim=1)*nB),3_I32P) ; if (padd>0_I4P) padd = 3_I32P - padd           ! computing the number of padding characters
  call encode_bits(bits=nI1P,padd=padd,code=code)                                      ! encoding bits
  return
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  implicit none
  integer(I32P),              intent(IN)::  nB                                 !< Number of bytes of single element of n.
  integer(I64P),              intent(IN)::  n(1:)                              !< Array of numbers to be encoded.
  character(1), allocatable, intent(OUT):: code(:)                            !< Encoded array.
  integer(I8P)::                           nI1P(1:((size(n,dim=1)*nB+2)/3)*3) !< One byte integer array containing n.
  integer(I32P)::                           padd                               !< Number of padding characters ('=').
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  implicit none
  integer(I32P),              intent(IN)::  nB                                 !< Number of bytes of single element of n.
  integer(I32P),              intent(IN)::  n(1:)                              !< Array of numbers to be encoded.
  character(1), allocatable, intent(OUT):: code(:)                            !< Encoded array.
  integer(I8P)::                           nI1P(1:((size(n,dim=1)*nB+2)/3)*3) !< One byte integer array containing n.
  integer(I32P)::                           padd                               !< Number of padding characters ('=').
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  implicit none
  integer(I32P),              intent(IN)::  nB                                 !< Number of bytes of single element of n.
  integer(I16P),              intent(IN)::  n(1:)                              !< Array of numbers to be encoded.
  character(1), allocatable, intent(OUT):: code(:)                            !< Encoded array.
  integer(I8P)::                           nI1P(1:((size(n,dim=1)*nB+2)/3)*3) !< One byte integer array containing n.
  integer(I32P)::                           padd                               !< Number of padding characters ('=').
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  if (allocated(code)) deallocate(code) ; allocate(code(1:((size(n,dim=1)*nB+2)/3)*4)) ! allocating code chars
  nI1P = transfer(n,nI1P)                                                              ! casting n to integer array of 1 byte elem
  padd = mod((size(n,dim=1)*nB),3_I32P) ; if (padd>0_I32P) padd = 3_I32P - padd           ! computing the number of padding characters
  call encode_bits(bits=nI1P,padd=padd,code=code)                                      ! encoding bits
  return
  !---------------------------------------------------------------------------------------------------------------------------------
